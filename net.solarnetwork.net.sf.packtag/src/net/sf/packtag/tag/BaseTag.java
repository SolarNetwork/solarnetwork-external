...
public abstract class BaseTag extends BodyTagSupport {
	...
	/**
	 * The name of the request attribute, that keeps track of the delivered
	 * resources
	 */
	...
	/**
	 * Configuration; returns the path, where the minifed resources will be put
	 * in (when cache type is file)
	 */
	...
	/**
	 * Configuration; returns the (virtual) path, where the combined resources
	 * will be fetched from (when cache type is servlet)
	 */
	protected String getCacheServletCombinedPath(final String strategyName) {
		return ContextConfiguration.getCacheServletCombinedPath(pageContext.getServletContext(),
				strategyName);
	}

	/**
	 * Configuration; returns true, when the resources just should be written
	 * out
	 */
	...
	/**
	 * Configuration; returns true, if the timestamps of the files should be
	 * checked each time, they are requested
	 */
	...
	/**
	 * Configuration; returns true, if the errors should not result in a HTTP
	 * 500
	 */
	...
	/**
	 * Configuration; returns true, if the resources should be keept in the
	 * request, in order to not write them out twice
	 */
	...
	/**
	 * Configuration; returns true, if external resources should be downloaded,
	 * minifed and compressed.
	 */
	...
	protected String getPackStrategyClassName(final String resourceType) {
		return ContextConfiguration.getPackStrategyClassName(pageContext.getServletContext(),
				resourceType);
	}
	...
	/**
	 * Throws an error directly (500), or hides it away (the exception will be
	 * printed as stacktrace).
	 */
	...
	/**
	 * Parses the requests with the Apache evaluation standard library (if
	 * availiable)
	 */
	protected Object evaluate(final String attributeName, final String expression,
			final Class expectedType) {
		throw new UnsupportedOperationException("Apache Taglibs evaulation not supported.");
	}

	/**
	 * Parses the body of the Tag, this approach doesn't need any XML Parser and
	 * is therefore probably faster (and better it doesn't has dependencies to
	 * those). BUT it is therefore fragile (No comments (if they are in the
	 * src-tag) nor whitespaces are allowed in the tags (e.g. &lt;src &gt;
	 * wouldn't work).
	 * 
	...
	/**
	 * Returns true if the classes from the Apache Standard Taglibrary are
	 * available
	 */
	protected boolean isStandardTaglibAvailable() {
		if ( standardTaglibAvailable == null ) {
			synchronized ( BaseTag.class ) {
				if ( standardTaglibAvailable == null ) {
					try {
						standardTaglibAvailable = new Boolean(
								Class.forName("org.apache.taglibs.standard.lang.support.ExpressionEvaluatorManager") != null);
