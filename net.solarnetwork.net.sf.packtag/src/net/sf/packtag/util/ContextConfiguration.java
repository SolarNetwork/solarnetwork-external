...
/**
 * Keeps track of the configuration settings in the /WEB-INF/packtag.properties
 * and /WEB-INF/packtag.user.properties file. Note: The configuration is not
 * longet stored in the web.xml.
 *
...
public class ContextConfiguration {
	...
	protected static Properties loadPropertyFile(final ServletContext context) {
		...
		try {
			...
			SafeLogger.error(ContextConfiguration.class,
					"Could not load file: /WEB-INF/packtag.properties", ex);
		}
		try {
			...
			SafeLogger.error(ContextConfiguration.class,
					"Could not load file: /WEB-INF/packtag.user.properties", ex);
		}
		return result;
	}
	...
	/**
	 * Should the files timestamp be checked, so the resource is renewed on
	 * change?
	 */
	...
	/**
	 * Should errors be shown, or hidden in the dark, resulting in unexpected
	 * behaviour/layout? (Not recommended)
	 */
	...
	public static String getCacheServletCombinedPath(final ServletContext context,
	...
	public static Class getCacheProviderClass(final ServletContext context) {
		Class result = null;
		String cacheProvider = getProperty(context, "cache.provider",
